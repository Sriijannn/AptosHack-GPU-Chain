module gpu_chain::compute_reward {
    ...
    // Platform treasury to hold coins
    ...
    // Create a task with APT payment
    public entry fun create_task_with_payment(
        requester: &signer,
        task_hash: String,
        deadline: u64,
        payment: Coin<AptosCoin>,
        admin_addr: address,
    ) acquires GlobalState, EventStore, PlatformTreasury {
        ...
        assert!(payment_amount >= MIN_REWARD, error::invalid_argument(E_INSUFFICIENT_PAYMENT));
        
        // Calculate platform fee
        let platform_fee = (payment_amount * PLATFORM_FEE_PERCENT) / 100;
        let reward_amount = payment_amount - platform_fee;
        ...
        global_state.platform_fees = global_state.platform_fees + platform_fee;
    ...
    public entry fun claim_rewards(
        worker: &signer,
        admin_addr: address,
    ) acquires GlobalState, PlatformTreasury {
        let worker_addr = signer::address_of(worker);
        let global_state = borrow_global_mut<GlobalState>(admin_addr);
        let treasury = borrow_global_mut<PlatformTreasury>(admin_addr);
        ...
        // Transfer APT from treasury to worker
        let reward_coins = coin::extract(&mut treasury.coins, reward_amount);
        coin::deposit(worker_addr, reward_coins);
    }

    // View functions (same as before)
    #[view]
